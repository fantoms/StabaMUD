...
/*Graveyard*/
/********************************************************************/
//These Specials need rewriting and the need to be assigned for zone 90 Graveyard
...
SPECIAL(grave_ghoul)
{
  if ((!AWAKE(ch) || !FIGHTING(ch))) return (FALSE);
...
SPECIAL(grave_priest)
{
  ...
  if ((number(0,4) || !AWAKE(smob))) return (FALSE);
